...
func initZerolog() {
	...
	// Stackdriver or console configuration
	stackdriver := bool(strings.ToLower(os.Getenv("ZEROLOG_CONFIG")) == "stackdriver")

	if stackdriver {
		log = zerolog.New(os.Stderr).Hook(severityHook{}).With().Timestamp().Caller().Logger()
		log.Info().Msg("Zerolog started with STACKDRIVER config")
	} else {
